...
public class JniEnvCallGen extends OpGenerator {
	public JniEnvCallGen(Semcode semc, OpRecognizer opr, Type typeIn, Type typeOut, String env_addr,
		...
		ins = fac.createLoadStoreInst(pEnv /* dest */, InstType.loadInst,
				...
				Arrays.asList(new Constant[] { new LocalVariable(env_addr),
		...
		List<Constant> operands = Arrays.asList(new Constant[] { ppFunc, new LocalVariable(env_addr) });
		if (args != null)
			operands.addAll(args);
		
		List<Type> types = Arrays.asList(new Type[] { funcType.getRetType(), LLVM2Jni.envTypePtrPtr });
		if (argTypes != null)
