...
llvm_program returns [Module program]
@init{
...
$globalVariable = valueFactory.createGlobalVariable(name, linkage, isThreadLocal, isConstant, type, 
    ...
    (initializer {initializer = $initializer.constant;} 
      (',' llvm_section {section = $llvm_section.section;}
      )? 
      (',' llvm_align {align = $llvm_align.align;})?
    )? ;
...
$function = valueFactory.createFunction(linkage, visibility, cconv, pAttributes, type, name, argList, isVararg, fAttributes, align, basicBlocks);  
    ...
    :
      ...
      GLOBAL_VARIABLE {name = $GLOBAL_VARIABLE.text; name = name.substring(1, name.length());}
      '(' argument_list[argList] {isVararg = $argument_list.isVarargFunction;}')'
      (UNNAMED_ADDR | FUNCTION_ATTRIBUTES {fAttributes.add($FUNCTION_ATTRIBUTES.text);})* 
      (llvm_align {align = $llvm_align.align;})?
      '{' 
...
//  INSTRUCTIONS
/////////////////////////////////////////////////////////////
llvm_align returns [int align]
    : ALIGN INTEGER {$align = Integer.parseInt($INTEGER.text);};

llvm_section returns [String section]
...
operator returns [Instruction inst]
    : 
    ( binary_op { $inst = $binary_op.inst; }
    | vector_op { $inst = $vector_op.inst; }
    | aggregate_op { $inst = $aggregate_op.inst; }
    | memory_op { $inst = $memory_op.inst; }
    | conversion_op { $inst = $conversion_op.inst; }
    | icmp_op { $inst = $icmp_op.inst; }
    | fcmp_op { $inst = $fcmp_op.inst; }
    | select_op { $inst = $select_op.inst; }
    | call_op { $inst = $call_op.inst; }
    )    
    dbg_metadata?
    ;
...
$inst = instFactory.createSimpleInst(dest, opcode, operands, types);
    ...
    (',' llvm_align {operands.add(valueFactory.createConstantValue(SimpleConstantValue.intConst, $llvm_align.align + ""));})?
    ;
...
expression returns [Constant expr]
    ...
    | metadata_expr {$expr = $metadata_expr.expr;}
    ;
...
metadata_definition :
	metadata_variable '=' metadata_expr;

metadata_variable : '!' (IDENTIFIER | INTEGER);
metadata_expr returns [Constant expr]
@init{
List<Argument> argList=new ArrayList<Argument>();
}
@after{
$expr = valueFactory.createVariable("undef");
}
    : '!' (STRING | INTEGER | '{' metadata_list '}')
    | metadata_variable
    | argument_list [argList] // list of <type> <value> pairs
    | NULL
    ;
...
initializer returns [Constant constant] 
    ...
    | llvm_constant {$constant = $llvm_constant.constant;}
...
llvm_constant returns [Constant constant]
    ...
    | undef_constant {$constant = $undef_constant.constant;};
...
undef_constant returns [Constant constant]
    : 'undef' { $constant = valueFactory.createVariable("undef"); };
